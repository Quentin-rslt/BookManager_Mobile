...
export default function TextIconButton({callBack, nameIcon: name, size, color, containerStyle, textStyle, text, showIcon = true, showText = true, isLoading = false, numberOfLineText= 2} : Props) {
    return (
        <TouchableOpacity onPress={callBack}>
            {
                isLoading ? <ActivityIndicator size="small" color={COLORS.background} style={containerStyle}/> :
